...
class InvalidLanguageError extends Error {
  constructor(lang: string) {
    super(`${lang} is not a valid language parameter.`);
    ...
    switch (lang) {
      ...
      default:
        throw new InvalidLanguageError(
          `${lang} is not a valid language parameter.`,
        );
    }
  } catch (error) {
...
export function instructionTexts(
  ...
  try {
    ...
    console.error((error as InvalidLanguageError).message);
    return { title: '', example: '', btn_next: '', btn_previous: '' };
  }
}
  ...
  try {
    switch (lang) {
      case 'en':
        return {
          preamble:
            'Experiment is in progress.<br> Do you really want to quit?',
